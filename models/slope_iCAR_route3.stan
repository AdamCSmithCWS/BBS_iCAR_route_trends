...
parameters {
  ...
  vector[nroutes] alpha_raw_space;
  vector[nroutes] alpha_raw_rand;
  real ALPHA; 
 ...
 real<lower=0> sdbeta_rand;    // sd of slopes 
  ...
  real<lower=0> sdalpha_rand;    // sd of intercepts
...
// covariate effect on intercepts and slopes
   ...
   alpha_space = (sdalpha_space*alpha_raw_space);
   alpha_rand = (sdalpha_rand*alpha_raw_rand);
   
   alpha = alpha_space + alpha_rand + ALPHA;
  ...
  sdalpha_space ~ gamma(2,0.1);//~ normal(0,0.05); //boundary avoiding prior on sd of slope spatial variation
  sdalpha_rand  ~ gamma(2,0.1);//~ normal(0,0.05); //boundary avoiding prior on sd of slope random variation
//  sdalpha ~ normal(0,1); //prior on sd of intercept variation
  ...
  alpha_raw_space ~ icar_normal_lpdf(nroutes, node1, node2);
