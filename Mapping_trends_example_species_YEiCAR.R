...
out_base_YEiCAR <- paste0(species_f,"YE_iCAR",firstYear,"_",lastYear)

csv_files_YEiCAR <- paste0(output_dir,"/",out_base_YEiCAR,"-",1:3,".csv")

stanfit_YEiCAR <- as_cmdstan_fit(files = csv_files_YEiCAR)

if(check_conv){
  
  stanf_df_YEiCAR <- stanfit_YEiCAR$draws(format = "df")
  
  
  conv_summ <- summarise_draws(stanf_df_YEiCAR) %>% 
    mutate(species = species,
           model = out_base_YEiCAR)
  conv_summary <- bind_rows(conv_summary,conv_summ)
